  ...
  try {
    ...
    if (!no_cache) {
      ...
      if (cached) {
        ...
        // 缓存无 summary/tags，先返回基础数据，Dify 放后台补全
        waitUntil((async () => {
    ...
    // 4. Dify AI 摘要 + 缓存写入放后台
    waitUntil((async () => {
      const aiMeta = await generateAiMeta(title, description, url, lang);
      await writeCache({
        url, title, description,
        og_image: meta.image || null, favicon_url: meta.favicon || null,
        site_name: meta.siteName || null, theme_color: meta.themeColor || null,
        summary: aiMeta.summary, tags: aiMeta.tags,
        fetched_at: new Date().toISOString(),
      });
    })());
