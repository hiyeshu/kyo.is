...
### components/
ChatApp.tsx: 聊天应用主组件，恢复最新 channel 历史并过滤旧空 assistant 壳子，管理 channelId、图片附件，对接 /api/agent/chat 的 Mastra agent 流，登录前置门禁，解析错误帧与 clientEffects，工具写入后触发读云刷新
ChatMessages.tsx: 消息列表组件，展示有可见 payload 的聊天历史（文本+图片）、自动滚动、加载状态
...
3. SSE 响应格式：0:"text" 文本增量，3:"error" 错误帧，d:{} 完成信号
