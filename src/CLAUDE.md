...
main.tsx: 应用入口，React 19 渲染，i18n 初始化，主题水合，资源预加载
...
lib/ - 工具库，i18n 翻译、Three.js 着色器、音频处理、链接元数据客户端
mastra/ - Agent 编排层，DeepSeek 模型、typed tools、Kyo agent 工厂
server/ - 服务端产品边界，HTTP、Supabase、channel、DeepSeek 分类逻辑
...
worker/ - Cloudflare Worker 入口，承接 /api、静态资源、SPA fallback
...
- 依赖 Zustand 全局状态管理
- 依赖 i18next 国际化
- 依赖 Cloudflare Worker /api 与 Mastra agent 通信
- 被 index.html 加载，作为 SPA 入口

## 架构约束
1. 所有应用必须通过 appRegistry 注册
2. 窗口管理统一使用 useWindowManager hook
3. 浏览器本地文件仍走 IndexedDB 工具；agent 文件系统必须走 workspace_files tool
...
9. Agent 写入必须经过 mastra/tools typed tool，不允许 UI 或模型直写数据库
